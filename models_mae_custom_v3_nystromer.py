...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
        ...
        # self.highres_decoder_blocks = nn.ModuleList([
        #     LinformerSelfAttention(dim=decoder_embed_dim, seq_len=highres_num_patches+1, heads=decoder_num_heads, k=256, one_kv_head=True, share_kv=True)
        #     for i in range(decoder_depth)
        # ])

        # self.highres_decoder = Performer(
        ...
        self.highres_decoder = Nystromformer(
            dim = decoder_embed_dim,
            dim_head = decoder_embed_dim//decoder_num_heads,
            heads = decoder_num_heads,
            depth = decoder_depth,
...
#        x_ = torch.gather(x_, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, x.shape[2]))  # unshuffle (128, 196, 512)
        ...
        x = self.highres_decoder(x, mask=attention_mask)
